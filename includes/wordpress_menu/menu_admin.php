...
function ausbildung_bewerbung_form_handler() {
    if (isset($_POST['ausbildung_bewerbung_submit'])) {
        $errors = array();

        // Daten sammeln und validieren
        $ausbildung_bewerbung_vorname = sanitize_text_field($_POST['ausbildung_bewerbung_vorname']);
        $ausbildung_bewerbung_nachname = sanitize_text_field($_POST['ausbildung_bewerbung_nachname']);
        $ausbildung_bewerbung_strasse = sanitize_text_field($_POST['ausbildung_bewerbung_strasse']);
        $ausbildung_bewerbung_ort = sanitize_text_field($_POST['ausbildung_bewerbung_ort']);
        $ausbildung_bewerbung_telefon = sanitize_text_field($_POST['ausbildung_bewerbung_telefon']);
        $ausbildung_bewerbung_email = sanitize_email($_POST['ausbildung_bewerbung_email']);
        $ausbildung_bewerbung_nachricht = sanitize_textarea_field($_POST['ausbildung_bewerbung_nachricht']);

        // Kontaktpersonen-E-Mail aus dem Formular abrufen
        $ausbildung_contact_person_email = sanitize_email($_POST['ausbildung_contact_person_email']);

        // Validierung der E-Mail-Adresse
        if (!is_email($ausbildung_bewerbung_email) || !is_email($ausbildung_contact_person_email)) {
            $errors[] = 'Ungültige E-Mail-Adresse.';
        }
        ...
        if (!empty($errors)) {
            echo '<script>
                    document.getElementById("ausbildung-bewerbung-error-message").innerHTML = "' . implode('<br>', $errors) . '";
                    document.getElementById("ausbildung-bewerbung-error-message").style.display = "block";
                  </script>';
            return;  // Verarbeitung abbrechen
        }

        // E-Mail-Adressen und Betreff
        $ausbildung_bewerbung_email_1 = $ausbildung_contact_person_email;
        $ausbildung_bewerbung_subject = 'Neue Ausbildung Bewerbung von ' . $ausbildung_bewerbung_vorname . ' ' . $ausbildung_bewerbung_nachname;

        // E-Mail-Inhalte
        $ausbildung_bewerbung_application_message = "<html><body>";
        $ausbildung_bewerbung_application_message .= "<h2>Neue Ausbildung Bewerbung</h2>";
        $ausbildung_bewerbung_application_message .= "<p><strong>Vorname:</strong> $ausbildung_bewerbung_vorname</p>";
        $ausbildung_bewerbung_application_message .= "<p><strong>Nachname:</strong> $ausbildung_bewerbung_nachname</p>";
        $ausbildung_bewerbung_application_message .= "<p><strong>Straße und Hausnummer:</strong> $ausbildung_bewerbung_strasse</p>";
        $ausbildung_bewerbung_application_message .= "<p><strong>PLZ, Wohnort:</strong> $ausbildung_bewerbung_ort</p>";
        $ausbildung_bewerbung_application_message .= "<p><strong>Telefonnummer:</strong> $ausbildung_bewerbung_telefon</p>";
        $ausbildung_bewerbung_application_message .= "<p><strong>Email:</strong> $ausbildung_bewerbung_email</p>";
        $ausbildung_bewerbung_application_message .= "<p><strong>Nachricht:</strong><br>$ausbildung_bewerbung_nachricht</p>";
        $ausbildung_bewerbung_application_message .= "</body></html>";

        // E-Mail-Header
        $ausbildung_bewerbung_headers = array(
            'Content-Type: text/html; charset=UTF-8',
            'From: Bewerbung <noreply@hbwa.de>',
        );

        // Dateien verarbeiten und Anhänge hinzufügen
        $ausbildung_bewerbung_attachments = array();
        $ausbildung_bewerbung_upload_dir = wp_upload_dir();

        for ($i = 1; $i <= 2; $i++) {
            if (!empty($_FILES['dateien' . $i]['name'])) {
                $ausbildung_bewerbung_uploaded_file = $_FILES['dateien' . $i];
                if (is_uploaded_file($ausbildung_bewerbung_uploaded_file['tmp_name'])) {
                    $ausbildung_bewerbung_upload_file_path = $ausbildung_bewerbung_upload_dir['path'] . '/' . basename($ausbildung_bewerbung_uploaded_file['name']);
                    if (move_uploaded_file($ausbildung_bewerbung_uploaded_file['tmp_name'], $ausbildung_bewerbung_upload_file_path)) {
                        $ausbildung_bewerbung_attachments[] = $ausbildung_bewerbung_upload_file_path;
                    }
                }
            }
        }
        ...
        // Bewerbung senden
        if (!wp_mail($ausbildung_bewerbung_email_1, $ausbildung_bewerbung_subject, $ausbildung_bewerbung_application_message, $ausbildung_bewerbung_headers, $ausbildung_bewerbung_attachments)) {
            echo '<script>
                    document.getElementById("ausbildung-bewerbung-error-message").innerHTML = "Es gab ein Problem beim Senden Ihrer Bewerbung.";
                    document.getElementById("ausbildung-bewerbung-error-message").style.display = "block";
                  </script>';
            return;  // Verarbeitung abbrechen
        }
...
add_action('init', 'ausbildung_bewerbung_form_handler');
